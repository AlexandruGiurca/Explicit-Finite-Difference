...
class Grid(object):
    ...
    def __init__(self, f_vol, f_value, f_time, i_nas, i_nts=None):
        ...
        '''
        # initialize variables and uses vol to ensure stability
        self.f_nas = 1. * i_nas
        # 'infinito' eh duas vezes o valor
        self.dS = 2 * f_value / self.f_nas
        # how wilmott guarantees satbility
        ...
        # initialize grid. The starting time point is the end
        self.grid = {}
        for k in range(int(self.i_nts) + 1):
            for i in range(int(self.f_nas) + 1):
                node = GridNode(i, k)
                self.grid[node] = node
...
class Derivative(object):
    ...
    def compare_to_analytical_solutions(self, l_S, f_time):
        ...
        '''
        d_price = {u'analytical': [], u'finite differences': []}
        d_delta = {u'analytical': [], u'finite differences': []}
        d_gamma = {u'analytical': [], u'finite differences': []}
        l_prices = l_S
        for f_S in l_prices:
            # prices
            f_aux = self.get_information(f_S, f_time, 'price_anlt')
            d_price[u'analytical'].append(f_aux)
            f_aux = self.get_information(f_S, f_time, 'price')
            d_price[u'finite differences'].append(f_aux)
            # calcula delta
            f_aux = self.get_information(f_S, f_time, 'delta_anlt')
            d_delta[u'analytical'].append(f_aux)
            f_aux = self.get_information(f_S, f_time, 'delta')
            d_delta[u'finite differences'].append(f_aux)
            # calcula gamma
            f_aux = self.get_information(f_S, f_time, 'gamma_anlt')
            d_gamma[u'analytical'].append(f_aux)
            f_aux = self.get_information(f_S, f_time, 'gamma')
            d_gamma[u'finite differences'].append(f_aux)
        # plot results
        ...
        for d_aux, ax, s_title in zip([d_price, d_delta, d_gamma],
            ...
            # df_plot.plot(ax=ax)
            ax.set_xlabel(u'Underlying Price')
            ax.set_title(s_title)

        ax1.set_ylabel(u'Value')
        s_prep = u"Comparison of Results for {}\n"
    ...
    def _set_final_condition(self):
        ...
        '''
        # only the final value of the asset is needed here
        for i in range(int(self.grid.f_nas) + 1):
        ...
        # fill in the last value of all columns (time)
        for j in range(int(self.grid.i_nts) + 1):
            f_S = i * 1. * self.grid.dS
    ...
    def _go_backwards(self):
        ...
        # start the loop after the first step of each dimension
        for k in range(1, int(self.grid.i_nts) + 1):
            for i in range(1, int(self.grid.f_nas)):
                ...
                # calculates Greeks by central difference
    ...
    def _get_matrix(self):
        ...
        '''
        # creates price and time lists
        l_time = [self.grid.dt * i for i in range(int(self.grid.i_nts) + 1)]
        l_price = [self.grid.dS * i for i in range(int(self.grid.f_nas) + 1)]
        ...
        # extrain information
        for k in range(int(self.grid.i_nts) + 1):
            for i in range(int(self.grid.f_nas) + 1):
                valid_node = self(i, k)
                d_rtn['asset'].loc[i, k] = valid_node.f_asset_value
                d_rtn['opt_prices'].loc[i, k] = valid_node.f_option_value
                d_rtn['delta'].loc[i, k] = valid_node.f_delta
                d_rtn['gamma'].loc[i, k] = valid_node.f_gamma
                d_rtn['theta'].loc[i, k] = valid_node.f_theta
                f_aux = valid_node.f_option_value_anlt
                d_rtn['opt_prices_anlt'].loc[i, k] = f_aux
                d_rtn['delta_anlt'].loc[i, k] = valid_node.f_delta_anlt
                d_rtn['gamma_anlt'].loc[i, k] = valid_node.f_gamma_anlt
...
class SquaredLogContract(Derivative):
    ...
    def __init__(self, f_St, f_sigma, f_time, f_r, i_nas, f_K=None, i_nts=None,
        ...
        super(SquaredLogContract, self).__init__(f_St=f_St,
                                                 ...
                                                 i_nts=i_nts,
                                                 f_sigmam=f_sigmam)
        self.s_name = 'Squared Log Contract'
        self._go_backwards()
        self._set_all_matrix()
...
class SquaredExotic(Derivative):
    '''
    A representation of a exotic sqared contract. The Strike is given
    '''
    def __init__(self, f_St, f_sigma, f_time, f_r, i_nas, f_K=None, i_nts=None,
                 f_sigmam=None):
        '''
    ...
    def _apply_boundary_conditions(self, k):
        ...
        f_rtn2 = 2 * self(i_nas - 1, k).f_option_value
        f_rtn2 -= self(i_nas - 2, k).f_option_value
        # adapting condition to the contract
        f_rtn2 += (2 * dS**2)
        return f_rtn1, f_rtn2
...
class EuropeanCallButterfly(Derivative):
    '''
    A representation of a european Call Butterfly strategy with legs expiring
    ...
    def __init__(self, f_St, f_sigma, f_time, f_r, i_nas, l_K, l_Q,
        ...
        self.l_K = l_K
        self.l_Q = l_Q
        super(EuropeanCallButterfly, self).__init__(f_St=f_St,
                                                    f_sigma=f_sigma,
                                                    f_time=f_time,
                                                    f_r=f_r,
                                                    i_nas=i_nas,
                                                    ...
                                                    i_nts=None,
                                                    f_sigmam=f_sigmam)
        self.s_name = 'European Call Butterfly'
        self._go_backwards()
        self._set_all_matrix()
...
class Derivative_UVM(object):
    ...
    def plot_all_solutions(self, l_S, f_time=10e-6):
        ...
        '''
        d_price = {u'finite differences': []}
        d_delta = {u'finite differences': []}
        d_gamma = {u'finite differences': []}
        l_prices = l_S
        for f_S in l_prices:
            # calcula precos
            f_aux = self.get_information(f_S, f_time, 'price')
        ...
        ax1.set_ylabel(u'Value')
        s_prep = u'Solutions Obtained for {}\n'
        fig.suptitle(s_prep.format(self.s_name), fontsize=16, y=1.03)
        fig.tight_layout()
    ...
    def _go_backwards(self):
        ...
        # time loop
        for k in range(1, i_NTS):
            # asset loop
            for i in range(1, i_NAS-1):
...
class EuropeanVanillaUvm(Derivative_UVM):
    '''
    A representation of a strategy of european options that can use
    three legs at most
    '''
    def __init__(self, f_volm, f_volM, f_intrate, s_ptype, l_strike, l_qty,
                 f_expiration, f_NAS):
        '''
        Initiate a EuropeanVanillaUvm object. Sale all parametes as attrubutes
        :param f_volm: float. The minimum underline volatility to be observed
        :param f_volM: float. The maximum underline volatility to be observed
        :param f_intrate: float. The risk free intereset rate
        :param s_ptype: string. Pur ot Call. 'P' or 'C'
        ...
        :param f_NAS: float. Number of asset steps
        '''
        super(EuropeanVanillaUvm, self).__init__(f_volm=f_volm,
                                                 f_volM=f_volM,
                                                 f_intrate=f_intrate,
                                                 s_ptype=s_ptype,
                                                 l_strike=l_strike,
        ...
        if s_ptype == 'C':
            s_type = 'Call'
        self.s_name = 'Start with European {}: '.format(s_type)
        ...
        self.OPTION = EuropeanVanillaUvm
...
class AmericanVanillaUvm(Derivative_UVM):
    ...
    def __init__(self, f_volm, f_volM, f_intrate, s_ptype, l_strike, l_qty,
                 f_expiration, f_NAS):
        '''
        Initiate an AmericanVanillaUvm object. Sale all parametes as attrubutes
        :param f_volm: float. The minimum underline volatility to be observed
        :param f_volM: float. The maximum underline volatility to be observed
        :param f_intrate: float. The risk free intereset rate
        :param s_ptype: string. Pur ot Call. 'P' or 'C'
        ...
        if s_ptype == 'C':
            s_type = 'Call'
        self.s_name = 'Start with American {}: '.format(s_type)
        self.s_name += '[{:0.1f}, {:0.1f}, {:0.1f}]\n'
        self.s_name = self.s_name.format(*l_qty)
        self.df_price, self.df_delta, self.df_gamma = self._go_backwards()
